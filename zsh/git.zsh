# Git aliases — adapted verbatim from prezto's git module.
# https://github.com/sorin-ionescu/prezto/blob/master/modules/git/alias.zsh
# Helper functions used by some aliases live in zsh/git-functions.zsh.

# Log / status formats referenced by the aliases below.
typeset -g _git_log_medium_format='%C(bold)Commit:%C(reset) %C(green)%H%C(red)%d%n%C(bold)Author:%C(reset) %C(cyan)%an <%ae>%n%C(bold)Date:%C(reset)   %C(blue)%ai (%ar)%C(reset)%n%+B'
typeset -g _git_log_oneline_format='%C(green)%h%C(reset) %s%C(red)%d%C(reset)%n'
typeset -g _git_log_brief_format='%C(green)%h%C(reset) %s%n%C(blue)(%ar by %an)%C(red)%d%C(reset)%n'
typeset -g _git_status_ignore_submodules='none'
